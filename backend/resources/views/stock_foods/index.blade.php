            ...
            <div class="pb-4 shadow-lg rounded-lg">
                <nav>
                    ...
                    <ul class="w-full mb-4">
                        ...
                        <li class="text-lg text-center border-b-4 border-double border-gray-300 mx-4 px-4"><a class="" href="{{ route('user.stock_foods.index', ['id' => $stock->id]) }}">
                            {{ $stock->name }}
                        </a></li>
                        @endforeach
                    </ul>
                ...
                <div class="px-10  bg-gray-50 shadow-sm rounded-md py-4 ">
                    <table class="table-fixed w-full mb-4">
                        ...
                        @foreach($stock_foods as $stock_food)
                            ...
                            <td class="w-1/6">{{ $stock_food->food->name }}</td>
                            <td class="w-1/6">{{ $stock_food->food->volume * $stock_food->count }}{{ $stock_food->food->unit }}</td>
                            <td class="w-1/12 hidden md:table-cell">{{ $stock_food->food->protein * $stock_food->count}}g</td>
                            <td class="w-1/12 hidden md:table-cell">{{ $stock_food->food->fat * $stock_food->count}}g</td>
                            <td class="w-1/12 hidden md:table-cell">{{ $stock_food->food->carbohydrate * $stock_food->count}}g</td>
                            <td class="w-1/6">{{ $stock_food->register_date->format('m/d') }}</td>
                            <td class="w-1/6">
                                <form  action="{{ route('user.stock_foods.destroy', ['id'=>$stock->id, 'stock_food_id'=>$stock_food->id]) }}" method="POST" onsubmit="return confirmFunctionDelete()" >
                                @csrf
                                @method('DELETE')
